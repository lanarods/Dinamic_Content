...
function createList(object, list) 
{
	...
	// loop through all of the nodes within object
		...
		//}
			...
			if (nodeLevel == prevLevel) { // append the entry to the current list
			list.appendChild(listItem);
			}
			else if (nodeLevel > prevLevel) { // append the entry to a new nested list 
			...
			else if (nodeLevel < prevLevel) { // append the entry to a higher-level list 
